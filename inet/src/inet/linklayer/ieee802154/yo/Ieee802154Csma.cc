...
Define_Module(Ieee802154Csma);

void Ieee802154Csma::initialize(int stage)
{
...
void Ieee802154Csma::initializeMACAddress()
{
...
InterfaceEntry *Ieee802154Csma::createInterfaceEntry()
{
 ...
 */
void Ieee802154Csma::handleUpperPacket(cPacket *msg)
{
    ...
    EV_DETAIL << "Ieee802154Csma received a message from upper layer, name is " << msg->getName() << ", CInfo removed, mac addr=" << cInfo->getDestinationAddress() << endl;
...
void Ieee802154Csma::updateStatusIdle(t_mac_event event, cMessage *msg)
{
    switch (event) {
...
void Ieee802154Csma::updateStatusBackoff(t_mac_event event, cMessage *msg)
{
    switch (event) {
...
void Ieee802154Csma::flushQueue()
{
    // TODO:
    macQueue.clear();
}

void Ieee802154Csma::clearQueue()
{
    macQueue.clear();
}

void Ieee802154Csma::attachSignal(PCSMAFrame *mac, simtime_t_cref startTime)
{
    simtime_t duration = mac->getBitLength() / bitrate;
    mac->setDuration(duration);
}

void Ieee802154Csma::updateStatusCCA(t_mac_event event, cMessage *msg)
{
    switch (event) {
        case EV_TIMER_CCA: {
            ...
            /* 0: Data, 1: has lower Interest, 2: unknown Interest */
            if (isIdle) {
                // decrement CW according to priority (naive)
                if (priority == 0)
                    CW = CW - 2;
                else
                    CW = CW - 1;
                ...
                if (CW == 0){
                    ...
                    delete mac;
                    EV_DETAIL << " starting Nb. " << (initialCW-CW) <<" CCA timer." << endl;
                    startTimer(TIMER_CCA);
                    updateMacState(CCA_3);
                    radio->setRadioMode(IRadio::RADIO_MODE_RECEIVER);
                }
                ...
                // increment NB according to priority
                NB = NB + (priority + 1);
                //NB = NB + 1;
                CW = initialCW;
...
void Ieee802154Csma::updateStatusTransmitFrame(t_mac_event event, cMessage *msg)
{
...
void Ieee802154Csma::updateStatusWaitAck(t_mac_event event, cMessage *msg)
{
    assert(useMACAcks);
...
void Ieee802154Csma::manageMissingAck(t_mac_event    /*event*/, cMessage *    /*msg*/)
{
...
void Ieee802154Csma::updateStatusSIFS(t_mac_event event, cMessage *msg)
{
    assert(useMACAcks);
...
void Ieee802154Csma::updateStatusTransmitAck(t_mac_event event, cMessage *msg)
{
    assert(useMACAcks);
...
void Ieee802154Csma::updateStatusNotIdle(cMessage *msg)
{
 ...
 */
void Ieee802154Csma::executeMac(t_mac_event event, cMessage *msg)
{
    ...
    if (macState != IDLE_1 && event == EV_SEND_REQUEST) {
        ...
        switch (macState) {
            ...
            default:
                EV << "Error in Ieee802154Csma FSM: an unknown state has been reached. macState=" << macState << endl;
                break;
        }
    }
}

void Ieee802154Csma::manageQueue()
{
    if (macQueue.size() != 0) {
...
void Ieee802154Csma::updateMacState(t_mac_states newMacState)
{
 ...
 */
void Ieee802154Csma::fsmError(t_mac_event event, cMessage *msg)
{
...
void Ieee802154Csma::startTimer(t_mac_timer timer)
{
    if (timer == TIMER_BACKOFF) {
        ...
        /*if (NB == 0){
            unsigned backoffBoundary = ((unsigned)(simTime().dbl()/aUnitBackoffPeriod.dbl()) + 1) * aUnitBackoffPeriod.dbl();
            scheduleAt(backoffTime + backoffBoundary, backoffTimer);
        }
        else*/
        scheduleAt(backoffTime + simTime(), backoffTimer);
    }
    else if (timer == TIMER_CCA) {
        simtime_t ccaTime = simTime() + rxSetupTime + ccaDetectionTime;
        //int backoffBoundaryCca = ((int)(ccaTime.dbl()/aUnitBackoffPeriod.dbl()) + 1) * aUnitBackoffPeriod.dbl();
        scheduleAt(ccaTime, ccaTimer);
...
/*simtime_t Ieee802154Csma::scheduleBackoff()
{
    simtime_t backoffTime;
 ...
 */
void Ieee802154Csma::handleSelfMessage(cMessage *msg)
{
    ...
    if (msg == backoffTimer)
        ...
        EV << "Ieee802154Csma Error: unknown timer fired:" << msg << endl;
}
 ...
 */
void Ieee802154Csma::handleLowerPacket(cPacket *msg)
{
    if (msg->hasBitError()) {
...
void Ieee802154Csma::receiveSignal(cComponent *source, simsignal_t signalID, long value, cObject *details)
{
    Enter_Method_Silent();
...
cPacket *Ieee802154Csma::decapsMsg(PCSMAFrame *macPkt)
{
 ...
 */
cObject *Ieee802154Csma::setUpControlInfo(cMessage *const pMsg, const MACAddress& pSrcAddr)
{
