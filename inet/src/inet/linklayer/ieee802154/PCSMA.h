...
#ifndef __INET_PCSMA_H
#define __INET_PCSMA_H
...
#include "inet/linklayer/ieee802154/PCSMAFrame_m.h"
...
/**
 * @brief Priority-based CSMA Mac-Layer.
 *
 *
 * @author Amar Abane
 *
 */
class INET_API PCSMA : public MACProtocolBase, public IMACProtocol
{
  public:
    PCSMA()
        : MACProtocolBase()
  ...
  protected:
    typedef std::list<PCSMAFrame *> MacQueue;
    ...
    /** @name NDN tracked statistics.*/
    long nbTxDataFrames;
    long nbTxIntFrames;
    long nbDroppedIntFrames;
    long nbDroppedDataFrames;
    cHistogram backoffsToTxData;
    cHistogram backoffsToTxInt;
    cHistogram intDistanceStat;
    cHistogram txFailIntRStat;
    cHistogram dataDistanceStat;
    cHistogram txFailDataRStat;

    cOutVector vBackoffsToTxData;
    cOutVector vBackoffsToTxInt;
    std::string statFile = "/home/amar/pandas/data/BF-ML/allStats.csv";
    /*@}*/
    ...
    enum backoff_methods {
        ...
        EXPONENTIAL,
        /** @brief Experimental back-off time for NDN.*/
        NDN,
    };
    ...
    /** @brief NDN-related parameters.*/
    short ndnType;
    int IMaxBackoffs, DMaxBackoffs;
    int nhbrs;
  ...
  protected:
    ...
    void attachSignal(PCSMAFrame *mac, simtime_t_cref startTime);
    ...
    float getTxFailData();
    float getTxFailInt();
    void writeLog(int pktSeq, short pktType, int distance, short nbrNhbrs, float FSR, float txFailIntR, float txFailDataR, double boInt, double boData, int nBackoffs);
    /**********/

    virtual simtime_t scheduleBackoff();

    virtual cPacket *decapsMsg(PCSMAFrame *macPkt);
...
//  cObject* setDownControlInfo(cMessage * const pMsg, Signal * const pSignal);
  ...
  private:
     ...
     */
    PCSMA& operator=(const PCSMA&);
};

} // namespace inet

#endif // ifndef __INET_PCSMA_H
