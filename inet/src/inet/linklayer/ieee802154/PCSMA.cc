...
void PCSMA::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        nbTxDataFrames = 0;
        nbTxIntFrames = 0;
        nbDroppedIntFrames = 0;
        nbDroppedDataFrames = 0;
        vBackoffsToTxData.setName("boData");
        vBackoffsToTxInt.setName("boInt");

        /* for trace file */
        std::ofstream  out;
        out.open(statFile.c_str(), std::ios_base::trunc);
        out << "pktSeq,node,pktType,distance,nbrNhbrs,FSR,txFailIntR,txFailDataR,boInt,boData,nBackoffs" << endl;
        out.close();
        /******************/
        ...
        if (backoffMethodStr == "exponential") {
            ...
            IMaxBackoffs = par("IMaxBackoffs");
            DMaxBackoffs = par("DMaxBackoffs");
            nhbrs = par("nhbrs");
        }
        /* NDN */
        else if (backoffMethodStr == "ndn") {
            backoffMethod = NDN;
            macMinBE = par("macMinBE");
            macMaxBE = par("macMaxBE");
            IMaxBackoffs = par("IMaxBackoffs");
...
void PCSMA::finish()
{
    ...
    /* record NDN stats */
    recordScalar("nbTxDataFrames", nbTxDataFrames);
    recordScalar("nbTxIntFrames", nbTxIntFrames);
    recordScalar("nbDroppedDataFrames", nbDroppedDataFrames);
    recordScalar("nbDroppedIntFrames", nbDroppedIntFrames);
    recordScalar("neighbors", nhbrs);

    recordScalar("boData:mean", backoffsToTxData.getMean());
    recordScalar("boInt:mean", backoffsToTxInt.getMean());
    recordScalar("boData:min", backoffsToTxData.getMin());
    recordScalar("boInt:min", backoffsToTxInt.getMin());
    recordScalar("boData:max", backoffsToTxData.getMax());
    recordScalar("boInt:max", backoffsToTxInt.getMax());

    recordScalar("dInt:mean", intDistanceStat.getMean());
    recordScalar("dData:mean", dataDistanceStat.getMean());
    recordScalar("txFailIntR:mean", txFailIntRStat.getMean());
    recordScalar("txFailDataR:mean", txFailDataRStat.getMean());
}
...
void PCSMA::initializeMACAddress()
{
...
InterfaceEntry *PCSMA::createInterfaceEntry()
{
 ...
 */
void PCSMA::handleUpperPacket(cPacket *msg)
{
    //MacPkt*macPkt = encapsMsg(msg);
    PCSMAFrame *macPkt = new PCSMAFrame(msg->getName());
    macPkt->setBitLength(headerLength);
    IMACProtocolControlInfo *const cInfo = check_and_cast<IMACProtocolControlInfo *>(msg->removeControlInfo());
    EV_DETAIL << "PCSMA received a message from upper layer, name is " << msg->getName() << ", CInfo removed, mac addr=" << cInfo->getDestinationAddress() << endl;
    ...
    macPkt->setNdnType(ndnPacket->getType());
    macPkt->setNa(ndnPacket->getNa());

    macPkt->setPktSeq(ndnPacket->getSeqNo());
    macPkt->setHopCount(ndnPacket->getHopCount());
    /*-----*/
...
void PCSMA::updateStatusIdle(t_mac_event event, cMessage *msg)
{
    switch (event) {
        case EV_SEND_REQUEST:
            if (macQueue.size() <= queueLength) {
                macQueue.push_back(static_cast<PCSMAFrame *>(msg));
                EV_DETAIL << "(1) FSM State IDLE_1, EV_SEND_REQUEST and [TxBuff avail]: startTimerBackOff -> BACKOFF." << endl;
                updateMacState(BACKOFF_2);
                NB = 0;
                ndnType = static_cast<PCSMAFrame *>(macQueue.front())->getNdnType();
                //BE = macMinBE;
        ...
        case EV_FRAME_RECEIVED:
            ...
            sendUp(decapsMsg(static_cast<PCSMAFrame *>(msg)));
            nbRxFrames++;
            delete msg;
...
void PCSMA::updateStatusBackoff(t_mac_event event, cMessage *msg)
{
    switch (event) {
...
void PCSMA::flushQueue()
{
    // TODO:
    macQueue.clear();
}

void PCSMA::clearQueue()
{
    macQueue.clear();
}

void PCSMA::attachSignal(PCSMAFrame *mac, simtime_t_cref startTime)
{
    simtime_t duration = mac->getBitLength() / bitrate;
    mac->setDuration(duration);
}

void PCSMA::updateStatusCCA(t_mac_event event, cMessage *msg)
{
    switch (event) {
        case EV_TIMER_CCA: {
            ...
            if (isIdle) {
                ...
                if (priority == 0){
                    writeLog(
                            mac->getPktSeq(), mac->getNdnType(),
                            mac->getHopCount(), nhbrs,
                            mac->getNa(),
                            getTxFailInt(), getTxFailData(),
                            backoffsToTxInt.getMean(), backoffsToTxData.getMean(),
                            NB+1);
                    nbTxDataFrames++;
                    backoffsToTxData.collect(NB+1);
                    vBackoffsToTxData.record(NB+1);
                }else{
                    writeLog(mac->getPktSeq(), mac->getNdnType(), mac->getHopCount(), nhbrs, mac->getNa(), getTxFailInt(), getTxFailData(), backoffsToTxInt.getMean(), backoffsToTxData.getMean(), NB+1);
                    nbTxIntFrames++;
                    backoffsToTxInt.collect(NB+1);
                    vBackoffsToTxInt.record(NB+1);
                }
            }
            else{       // not idle
                NB = NB + 1;
                if ( ((priority == 1) && (NB >= IMaxBackoffs)) || ((priority == 0) && (NB >= DMaxBackoffs)) ){
                    // drop the frame
                    EV_DETAIL << "Dropping the packet." << endl;
                    cMessage *macc = macQueue.front();
                    macQueue.pop_front();
                    txAttempts = 0;
                    nbDroppedFrames++;
                    emit(packetFromUpperDroppedSignal, macc);
                    delete macc;
                    manageQueue();

                    if ( priority == 0 ){
                        writeLog(mac->getPktSeq(), mac->getNdnType(), mac->getHopCount(), nhbrs, mac->getNa(), getTxFailInt(), getTxFailData(), backoffsToTxInt.getMean(), backoffsToTxData.getMean(), -1);
                        nbDroppedDataFrames++;
                        vBackoffsToTxData.record(-1);
                    }else{
                        writeLog(mac->getPktSeq(), mac->getNdnType(), mac->getHopCount(), nhbrs, mac->getNa(), getTxFailInt(), getTxFailData(), backoffsToTxInt.getMean(), backoffsToTxData.getMean(), -1);
                        nbDroppedIntFrames++;
                        vBackoffsToTxInt.record(-1);
                    }
                }
                else{
...
void PCSMA::updateStatusTransmitFrame(t_mac_event event, cMessage *msg)
{
    if (event == EV_FRAME_TRANSMITTED) {
        //    delete msg;
        PCSMAFrame *packet = macQueue.front();
...
void PCSMA::updateStatusWaitAck(t_mac_event event, cMessage *msg)
{
    assert(useMACAcks);
...
void PCSMA::manageMissingAck(t_mac_event    /*event*/, cMessage *    /*msg*/)
{
...
void PCSMA::updateStatusSIFS(t_mac_event event, cMessage *msg)
{
    assert(useMACAcks);
...
void PCSMA::updateStatusTransmitAck(t_mac_event event, cMessage *msg)
{
    assert(useMACAcks);
...
void PCSMA::updateStatusNotIdle(cMessage *msg)
{
 ...
 */
void PCSMA::executeMac(t_mac_event event, cMessage *msg)
{
    ...
    if (macState != IDLE_1 && event == EV_SEND_REQUEST) {
        ...
        switch (macState) {
            ...
            default:
                EV << "Error in PCSMA FSM: an unknown state has been reached. macState=" << macState << endl;
                break;
        }
    }
}

void PCSMA::manageQueue()
{
    if (macQueue.size() != 0) {
...
void PCSMA::updateMacState(t_mac_states newMacState)
{
 ...
 */
void PCSMA::fsmError(t_mac_event event, cMessage *msg)
{
...
void PCSMA::startTimer(t_mac_timer timer)
{
...
simtime_t PCSMA::scheduleBackoff()
{
    ...
    switch (backoffMethod) {
        ...
        case NDN: {
            int BE = macMaxBE;
            if( ndnType == 5 ){
                int z = std::max(macMinBE, macMaxBE - NB);
                BE = std::min(z, macMaxBE);
            }else{
                BE = std::min(macMinBE + NB, macMaxBE);
            }
            int v = (1 << BE) - 1;
            int r = intuniform(0, v, 0);
            backoffTime = r * aUnitBackoffPeriod;
 ...
 */
void PCSMA::handleSelfMessage(cMessage *msg)
{
    ...
    if (msg == backoffTimer)
        ...
        EV << "PCSMA Error: unknown timer fired:" << msg << endl;
}
 ...
 */
void PCSMA::handleLowerPacket(cPacket *msg)
{
    if (msg->hasBitError()) {
    ...
    PCSMAFrame *macPkt = static_cast<PCSMAFrame *>(msg);
...
//                nbRxFrames++;
                ...
                if (ackMessage != nullptr)
                    delete ackMessage;
                ackMessage = new PCSMAFrame("CSMA-Ack");
                ...
                PCSMAFrame *firstPacket = static_cast<PCSMAFrame *>(macQueue.front());
...
void PCSMA::receiveSignal(cComponent *source, simsignal_t signalID, long value, cObject *details)
{
    Enter_Method_Silent();
...
cPacket *PCSMA::decapsMsg(PCSMAFrame *macPkt)
{
 ...
 */
cObject *PCSMA::setUpControlInfo(cMessage *const pMsg, const MACAddress& pSrcAddr)
{
...
float PCSMA::getTxFailData()
{
    if ((nbTxDataFrames + nbDroppedDataFrames) == 0)
        return 0;
    return (float)((float)nbDroppedDataFrames / (float)(nbTxDataFrames + nbDroppedDataFrames));
}

float PCSMA::getTxFailInt()
{
    if ((nbTxIntFrames + nbDroppedIntFrames) == 0)
        return 0;
    return (float)((float)nbDroppedIntFrames / (float)(nbTxIntFrames + nbDroppedIntFrames));
}


void PCSMA::writeLog(int pktSeq, short pktType, int distance, short nbrNhbrs, float FSR, float txFailIntR, float txFailDataR, double boInt, double boData, int nBackoffs)
{
    std::ofstream  out;
    out.open(statFile, std::ios_base::app);
    out << pktSeq << "," << getFullPath() << "," << pktType << "," << distance << "," << nbrNhbrs << "," << FSR << "," << txFailIntR << "," << txFailDataR << "," << boInt << "," << boData << "," << nBackoffs << endl;
    out.close();

    if(pktType == 5)
        intDistanceStat.collect(distance);
    else
        dataDistanceStat.collect(distance);

    txFailIntRStat.collect(txFailIntR);
    txFailDataRStat.collect(txFailDataR);
}
